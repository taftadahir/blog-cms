...
const form = useForm({
    new_sys_id: false,
    same_article_as: props.article.id,
    parent_id: props.article.parent_id,
    title: props.article.title,
    slug: props.article.slug,
    content: props.article.content,
    content_type: props.article.content_type,
    excerpt: props.article.excerpt,
    keywords: props.article.keywords,
    status: props.article.status,
    disable_comment: props.article.disable_comment == 1,
    disable_approval: props.article.disable_approval == 1,
    read_time: props.article.read_time,
    version: props.article.version,
})
watch(
    () => form.new_sys_id,
    (newValue, _) => {
...
const parentArticles = computed(() => {
    return props.articles.filter((article) => article.id != props.article.id)
})
const submit = () => {
        ...
        <form @submit.prevent="submit">
            ...
            <div class="mt-4">
                ...
                >
                    ...
                    <option
                        v-for="article in parentArticles"
                        :key="article.id"
                        :value="article.id"
                        v-text="article.title"
                    />
            ...
            <div class="mt-4">
                <InputLabel for="read_time" value="Read time" />
                <TextInput
                    id="read_time"
                    type="number"
                    class="mt-1 block w-full"
